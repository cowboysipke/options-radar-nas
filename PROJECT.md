...
从 Discord 的异常期权频道实时采集美股期权信号，合并多位分析师意见，对接行情源（IBKR/Massive/富途）做确定性评分筛选，每日输出 0-3 张候选合约，通过飞书群发送日报与提醒。系统只读不交易，模拟仓位和回测仅用于评估策略表现。
...
| 本地面板（Web UI） | ✅ | http://127.0.0.1:8787，首页推荐/信号/持仓/回测/系统诊断/配置 |
| 定时任务调度 | ✅ | 60s Discord 采集、5min IBKR 同步、17:15 ET 日报、18:00 ET 回测 |
