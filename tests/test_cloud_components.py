...
class BrokerMarketTests(unittest.TestCase):
    ...
    def test_massive_retries_transient_network_without_leaking_key(self):
        calls = []

        def flaky(url, timeout):
            calls.append(url)
            if len(calls) < 3:
                raise RuntimeError("massive_network_error?apiKey=secret")
            return {"results": []}
        ...
        payload = client.aggregate_bars("SPY", date(2026, 1, 1), date(2026, 1, 1))
        self.assertEqual(payload, [])
        self.assertEqual(len(calls), 3)
        self.assertIsNone(client.health()["last_error"])
        self.assertNotIn("secret", str(client.health()))

    def test_massive_health_probe_is_read_only_and_safe(self):
        requested = []
        ...
        client = MassiveClient(api_key="secret", http_get=fake, requests_per_minute=99)
        status = client.health(check_remote=True)
        self.assertEqual(status["status"], "ok")
        self.assertEqual(status["probe"], "read_only_aggregate")
        self.assertEqual(len(requested), 1)
        self.assertNotIn("secret", str(status))


class DiscordAndStorageTests(unittest.TestCase):
    def test_discord_open_login_reports_visible_flow(self):
        source = DiscordBrowserSource(Path("profile"), Path("evidence"), {"pa": "pa"}, headless=False)
        class FakePage:
        ...
        result = source.open_login()
        self.assertEqual(result["status"], "login_required")
        self.assertIn("登录窗口", result["message"])
