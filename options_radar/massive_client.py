...
def _default_get(url: str, timeout: float) -> Dict[str, object]:
    """Fetch a JSON document without ever including the API key in errors."""
    request = urllib.request.Request(url, headers={"User-Agent": "options-radar/0.2"})
    try:
        with urllib.request.urlopen(request, timeout=timeout) as response:
            return json.loads(response.read().decode("utf-8"))
    except urllib.error.HTTPError as exc:
        # The URL contains the key as a query parameter.  Do not propagate it
        # into logs or dashboard responses.
        raise RuntimeError(f"massive_http_{exc.code}") from exc
    except (urllib.error.URLError, TimeoutError, OSError) as exc:
        raise RuntimeError("massive_network_error") from exc
    except (ValueError, UnicodeError) as exc:
        raise RuntimeError("massive_invalid_json") from exc
...
class MassiveClient:
    ...
    def _get(self, path: str, params: Optional[Dict[str, object]] = None) -> Dict[str, object]:
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                if not isinstance(payload, dict):
                    raise RuntimeError("massive_invalid_response")
        ...
        raise RuntimeError(self._safe_error(last_error or RuntimeError("massive_unknown_error")))

    @staticmethod
    def _safe_error(exc: BaseException) -> str:
        text = str(exc).lower()
        if "http_" in text:
            match = text[text.find("http_"):].split()[0]
            return match[:64]
        if "timeout" in text:
            return "massive_timeout"
        if "network" in text or "url" in text or "connection" in text:
            return "massive_network_error"
        if "json" in text or "response" in text:
            return "massive_invalid_response"
        return "massive_request_error"

    @classmethod
    def _retryable(cls, exc: BaseException) -> bool:
        text = cls._safe_error(exc)
        if text in {"massive_timeout", "massive_network_error", "massive_invalid_response"}:
            return True
        if text.startswith("massive_http_"):
            try:
                code = int(text.rsplit("_", 1)[-1])
            except ValueError:
                return False
            return code == 429 or code >= 500
        return False

    def health(self, check_remote: bool = False) -> Dict[str, object]:
        """Return safe provider diagnostics; optionally perform one read-only probe."""
        result: Dict[str, object] = {
            "provider": "massive",
            "configured": bool(self.api_key),
            "status": "disabled" if not self.api_key else "configured",
        ...
        if check_remote:
            ...
            try:
                self._get(
                    "/v2/aggs/ticker/SPY/range/1/day/" + probe_day.isoformat() + "/" + probe_day.isoformat(),
                    {"adjusted": "true", "sort": "asc", "limit": 1},
                )
                result["status"] = "ok"
                result["probe"] = "read_only_aggregate"
            except Exception as exc:
                result["status"] = "error"
    ...
    def exact_snapshot(self, contract_key: str, as_of: Optional[date] = None) -> MarketSnapshot:
        ...
        try:
            ...
            return MarketSnapshot(
                contract_key=contract_key, observed_at=observed, provider="massive",
                data_status=f"error:{self._safe_error(exc)}",
